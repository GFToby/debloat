#-----------Settings-----------#
$UninstallPreInstalledOffice = "$true"
$UninstallWindowsApps = "$true"
#$ConfigureStartMenue = "$true"
$nstall7Zip = "$true"
#365
$Install365Business = "$false"
$Install365Enterprise = "$false"
#------------------------------#

Start-Transcript -Path "C:\Windows\Logs\Install.log" -Append

Set-ExecutionPolicy Unrestricted -Force

$DownLoad = "/download configuration.xml"
$Install = "/configure configuration.xml"

   #Avinstallerar förinstallerat O365
   if ($UninstallPreInstalledOffice -eq $true)
   {
   Write-Host Avinstallerar förinstallerat O365 -ForegroundColor Yellow
   $OfficeUninstallStrings = (Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "*365*"} | Select-Object UninstallString).UninstallString
   ...
   #Avinstallerar förinstallerade appar
   if ($UninstallWindowsApps -eq $true)
   {
   Write-Host Avinstallerar förinstallerade Windows appar -ForegroundColor Yellow
   ...
   Get-AppxPackage *xboxapp* -AllUsers | Remove-AppxPackage 
    ...
    #Konfiguration för Office (Microsoft 365 Apps for Business)
    $Buisiness = '
    <Configuration ID="ac2ff98b-8627-46c4-80d9-ea39994c8198">
  ...
  <Display Level="Full" AcceptEULA="TRUE" />
    ...
    #Konfiguration för Office (Microsoft 365 Apps for Enterprise)
    $Enterprise = '
    <Configuration ID="ac2ff98b-8627-46c4-80d9-ea39994c8198">
  <Add OfficeClientEdition="64" Channel="Current">
...
if ($Install365Business -eq "$true")
{
    #Hämtar Office deployment tool
    Write-Host Hämtar Office deployment tool -ForegroundColor Yellow
    $Temp = Test-Path C:\Temp
    if ($Temp -eq $false)
    {
        New-Item -Path C:\Temp -ItemType Directory
    }
    Invoke-WebRequest -uri "https://download.microsoft.com/download/2/7/A/27AF1BE6-DD20-4CB4-B154-EBAB8A7D4A7E/officedeploymenttool_14131-20278.exe" -OutFile c:\temp\officedeploymenttool_14131-20278.exe
    #Skapar konfigurationsfil för installation
    new-item C:\temp\configuration.xml 
    Add-Content -Path C:\temp\configuration.xml -Value $Buisiness

    #Kontrollerar ifall 7zip är installerat och installerar vid behov
    $zip = Test-Path -Path "C:\Program Files\7-Zip\7zFM.exe"

    if ($zip -eq $false)
    {
        Write-Host Hämtar och installerar 7zip -ForegroundColor Yellow
        Invoke-WebRequest -Uri "https://www.7-zip.org/a/7z1900-x64.msi" -OutFile c:\temp\7zip.msi
        $ArgumentList = "/quiet"
        Start-Process -FilePath c:\temp\7zip.msi -ArgumentList $ArgumentList -Wait           
    }


    #Packar upp Office deployment tool
    $zipfilePath = "c:\temp\officedeploymenttool_14131-20278.exe"
    $destinationUnzipPath = "C:\temp"
    & ${env:ProgramFiles}\7-Zip\7z.exe x $zipfilePath "-o$($destinationUnzipPath)" -y
    Write-Host Hämtar och installerar Office -ForegroundColor Yellow
    Start-Process .\setup.exe -ArgumentList $DownLoad -Wait
    Start-Process .\setup.exe -ArgumentList $Install -Wait
    Start-Process "C:\Program Files (x86)\Teams Installer\Teams.exe" -Wait
}

if ($Install365Enterprise -eq "$true")
{
    #Hämtar Office deployment tool
    Write-Host Hämtar Office deployment tool -ForegroundColor Yellow
    $Temp = Test-Path C:\Temp
    ...
    #Skapar konfigurationsfil för installation
    new-item C:\temp\configuration.xml 
    Add-Content -Path C:\temp\configuration.xml -Value $Enterprise
   ...
   # if ($ConfigureStartMenue -eq $true)
   # {
   # #Konfigurerar aktivitetsfältet och startmenyn
   # Write-Host Konfigurerar aktivitetsfältet och startmenyn -ForegroundColor Yellow
   # New-Item -Path HKLM:\Software\Policies\Microsoft\Windows\explorer
   # New-ItemProperty -path HKLM:\Software\Policies\Microsoft\Windows\Explorer -Name "StartLayoutFile" -Value "1" -PropertyType "Dword"
   # New-ItemProperty -path HKLM:\Software\Policies\Microsoft\Windows\Explorer -Name "LockedStartLayout" -Value "C:\temp\meny.xml" 
   # }




    #Startar eventuella uppdateringar 
    Write-Host Uppdaterar Windows. Stäng inte av datorn. -ForegroundColor Yellow
    ...
    #Rensar skrivbordet 
    $Desktop = get-item -Path $env:USERPROFILE\desktop\*
    foreach ($item in $Desktop)
    {
        Remove-Item -Path $item
    }

    #Läser in funktion för att sätta filassociation för Adobe och Outlook
...
function Set-PTA {
  ...
  Set-FTA -ProgId $ProgId -Protocol $Protocol -Icon $Icon
    ...
    #Kontrollerar ifall Adobe Acrobat är installerat och sätter som standard app för .pdf om så är fallet.
    $Adobe = (Get-WmiObject -Class Win32_Product).name
if ($Adobe -like "*Adobe Acrobat Reader*")
{
    Write-Host Adobe är installerat. Sätter Adobe som standard för .pdf -ForegroundColor Yellow
    Set-FTA AcroExch.Document.DC .pdf
}

    #Sätter Outlook som standardapp för .eml samt .msg
    $OfficeIsInstalled = (Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*).DisplayName 
if ($OfficeIsInstalled -like "Microsoft 365*")
{
    Write-Host Office är installerat. Sätter Outlook som standard för .msg samt för .eml -ForegroundColor Yellow   
    Set-FTA Outlook.File.msg.15 .msg
    Set-FTA Outlook.File.eml.15 .eml
}

    if ($nstall7Zip -eq $true)
    {
